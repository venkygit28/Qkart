  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChange={(e)=>handleNewAddress((prev)=>({
          ...prev,
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const res=await axios.post(`${config.endpoint}/user/addresses`,{address:newAddress.value},{ headers: { 'Authorization': `Bearer ${token}`,'Content-Type': 'application/json' } })
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      axios.delete(`${config.endpoint}/user/addresses/${addressId}`,{ headers: { 'Authorization': `Bearer ${token}`,'Accept': 'application/json, text/plain, */*' } })
      .then(res=>{
        //console.log(res);
        setAddresses((prev)=>({
            ...prev,
  ...
  const performCheckout = async (token, items, addresses) => {
    if(validateRequest(items,addresses)){
      axios.post(`${config.endpoint}/cart/checkout`,{"addressId":addresses.selected},{ headers: { 'Authorization': `Bearer ${token}`,'Content-Type': 'application/json' } })
      .then(res=>{
          //console.log(res)
            ...
            <Box>
              ...
              addresses.all.map(e=>{
                 return (
                        ...
                        <Box
                        className={addresses.selected===e._id?"address-item selected":"address-item not-selected"}
                        ...
                        >
                          ...
                          <Button name="delete/i" onClick={()=>deleteAddress(token,e._id)}><Delete /> DELETE</Button>
                        </Box>
